...
* **Parameter Estimation:** The Weibull function is linearized to fit a $Y=mX+q$ format, where $\beta$ is the slope and $\alpha$ is derived from the intercept. Linear regression is then applied to estimate the parameters for each batch.

## Results and Conclusions
Our analysis highlights the extreme sensitivity of the shape parameter ($\beta$). While the scale parameter ($\alpha$) stabilizes quickly, $\beta$ requires more data to converge.
...
* **Power Impact:** Using only 3 samples results in a 57.99% error in $\beta$ and a significant overestimation of extractable power (~191 kW vs. the stable ~172 kW).
* **Stability:** High-iteration counts ($M \ge 5000$) are necessary to eliminate numerical noise and validate these statistical thresholds.
